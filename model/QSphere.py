...
class emb(torch.nn.Module):
    ...
    def forward(self, dist, angle, torsion, idx_kj):
        ...
        angle_emb = self.angle_emb(dist, angle, idx_kj, if_q_angle=False)
...
class init(torch.nn.Module):
    def __init__(self, num_radial, hidden_channels, act=swish, use_node_features=True):
        ...
        if self.use_node_features:
            self.emb = Embedding(95, hidden_channels)  # [95, 128]
        ...
        self.lin_rbf_0 = Linear(num_radial, hidden_channels)  # (6, 128) a trivial linear layer
        self.lin = Linear(3 * hidden_channels, hidden_channels)  # (3 * 128, 128)
        self.lin_rbf_1 = nn.Linear(num_radial, hidden_channels, bias=False)  # (6, 128) without bias
        self.reset_parameters()
    ...
    def forward(self, x, emb, i, j):
        ...
        if self.use_node_features:
            x = self.emb(x)
            # print("x.shape", x.shape) # [num_node, 128]
        else:
            x = self.node_embedding[None, :].expand(x.shape[0], -1)  # [1, 128] -> [num_nodes, 128
        rbf0 = self.act(self.lin_rbf_0(rbf))  # [8336, 128]
        e1 = self.act(self.lin(torch.cat([x[i], x[j], rbf0], dim=-1)))  # [8336, 128]
        e2 = self.lin_rbf_1(rbf) * e1  # [8336, 128]
...
class update_e(torch.nn.Module):
    def __init__(self, hidden_channels, int_emb_size, basis_emb_size_dist, basis_emb_size_angle, basis_emb_size_torsion,
        ...
        self.lin_q1 = nn.Linear(num_spherical * num_radial, basis_emb_size_angle, bias=False)
        self.lin_q2 = nn.Linear(basis_emb_size_angle, hidden_channels, bias=False)
        ...
        self.lin_down1 = nn.Linear(hidden_channels, int_emb_size, bias=False)
        self.lin_down2 = nn.Linear(hidden_channels, int_emb_size, bias=False)
        self.lin_up1 = nn.Linear(int_emb_size, hidden_channels, bias=False)
        self.lin_up2 = nn.Linear(int_emb_size, hidden_channels, bias=False)
    ...
    def reset_parameters(self):
        ...
        glorot_orthogonal(self.lin_q1.weight, scale=2.0)
        glorot_orthogonal(self.lin_q2.weight, scale=2.0)
        ...
        glorot_orthogonal(self.lin_down1.weight, scale=2.0)
        glorot_orthogonal(self.lin_up1.weight, scale=2.0)
        glorot_orthogonal(self.lin_down2.weight, scale=2.0)
        glorot_orthogonal(self.lin_up2.weight, scale=2.0)

        glorot_orthogonal(self.lin_cat.weight, scale=2.0)
    ...
    def forward(self, x, emb, idx_kj, idx_ji, q_r):
        ...
        x_ji = self.act(self.lin_ji(x1))  # [8336, 128]
        x_kj = self.act(self.lin_kj(x1))

        rbf = self.lin_rbf1(rbf0)  # [8336, 8]
        rbf = self.lin_rbf2(rbf)  # [8336, 128]
        x_kj = x_kj * rbf  # [8336, 128]
        x_kj = self.act(self.lin_down1(x_kj))  # [8336, 64]

        sbf = self.lin_sbf1(sbf)  # [118466, 8]
        sbf = self.lin_sbf2(sbf)  # [118466, 64]
        x_kj = x_kj[idx_kj] * sbf  # [118466, 64]

        t = self.lin_t1(t)  # [118466, 8]
        t = self.lin_t2(t)  # [118466, 64]
        x_kj = x_kj * t  # [118466, 64]

        x_kj = scatter(x_kj, idx_ji, dim=0, dim_size=x1.size(0))
        # print("x_kj.shape", x_kj.shape) # [8336, 64]

        x_kj = self.act(self.lin_up1(x_kj))

        e1 = x_ji + x_kj  # local feature
...
class update_v(torch.nn.Module):
    def __init__(self, hidden_channels, out_emb_channels, out_channels, num_output_layers, act, output_init):
        ...
        self.lin_up = nn.Linear(hidden_channels, out_emb_channels, bias=True)  # [128, 256]
        self.lins = torch.nn.ModuleList()
        for _ in range(num_output_layers):
            self.lins.append(nn.Linear(out_emb_channels, out_emb_channels))  # [256, 256]
        self.lin = nn.Linear(out_emb_channels, out_channels, bias=False)  # [256, 1]
    ...
    def reset_parameters(self):
        ...
        for lin in self.lins:
            ...
            lin.bias.data.fill_(0)  # fill with zeros
    ...
    def forward(self, e, i):
        _, e2 = e  # [8336, 128]
        v = scatter(e2, i, dim=0)  # v[index[i]] = \sum src[i], a reduction operation
        v = self.lin_up(v)  # [560, 256]
        for lin in self.lins:
            v = self.act(lin(v))
        v = self.lin(v)
...
class update_u(torch.nn.Module):
    ...
    def forward(self, u, v, batch):
        # print("v.shape", v.shape) # [578, 1]
        # print("batch.shape", batch.shape) # [578]
        u += scatter(v, batch, dim=0)
        # print("u.shape", u.shape) # [32, 1]
        return u


class Q_SphereNet(torch.nn.Module):
    def __init__(
            ...
            act=swish, output_init='GlorotOrthogonal', use_node_features=True, n=30, sort_by_angle=False, num=-1):
        ...
        '''
        super(Q_SphereNet, self).__init__()
    ...
    def forward(self, batch_data):
        z, pos, batch = batch_data.z, batch_data.pos, batch_data.batch  # batch can reconstruct a single graph.
        if self.energy_and_force:
            pos.requires_grad_()
        edge_index = radius_graph(pos, r=self.cutoff, batch=batch)
        num_nodes = z.size(0)
        dist, angle, torsion, i, j, idx_kj, idx_ji, q_r = xyz_to_dat(pos, edge_index, num_nodes, num=self.num, n=self.n,
                                                                     sort_by_angle=self.sort_by_angle)
