...
class InteractionLayer(MessagePassing):
    def __init__(self, hidden_channels, middle_channels, num_radial, num_spherical, num_output_layers, output_channels,
        ...
        self.lin_rbf = nn.Sequential(Linear(9, hidden_channels), nn.SiLU())
        self.lin_pos = nn.Sequential(Linear(16, hidden_channels), nn.SiLU())
    ...
    def forward(self, x, pos, feature1, feature2, feature3, edge_index, edge_attr, batch):
        ...
        # h1 = self.propagate(edge_index=edge_index, x=x, edge_attr=edge_attr, base=rbf)
        # h1 = self.lin3(h1)
        # h1 = self.act(h1)

        tbf = self.lin_t(feature2)
        h2 = self.propagate(edge_index=edge_index, x=x, edge_attr=edge_attr, base=tbf)
        h2 = self.lin1(h2)
        h2 = self.act(h2)

        sbf = self.lin_s(feature3)
        h3 = self.propagate(edge_index=edge_index, x=x, edge_attr=edge_attr, base=sbf)
        h3 = self.lin2(h3)
        h3 = self.act(h3)

        p = self.lin_pos(pos)
        h4 = self.propagate(edge_index=edge_index, x=x, edge_attr=edge_attr, base=p)
        h4 = self.lin3(h4)
        h4 = self.act(h4)

        h = self.lin_cat(torch.cat([h2, h3, h4], 1))
        # h = self.lin_cat(h1)
        h = self.out(h + x)
        h = self.norm(h, batch)
        return h
...
class HomoNet2(nn.Module):
    def __init__(
            self, in_channels=48, cutoff=6.0, num_layers=3, hidden_channels=256, middle_channels=64, out_channels=1,
            num_radial=6,
            num_spherical=2,
            num_output_layers=2,
    ):
