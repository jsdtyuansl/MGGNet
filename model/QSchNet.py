...
class update_e(torch.nn.Module):
    def __init__(self, hidden_channels, num_filters, num_gaussians, cutoff):
        ...
        self.lin_cat = Linear(hidden_channels + 1, hidden_channels)
        self.mlp = Sequential(
    ...
    def forward(self, v, dist, dist_emb, edge_index, q_r):
        j, _ = edge_index
        C = 0.5 * (torch.cos(dist * PI / self.cutoff) + 1.0)
        W = self.mlp(dist_emb) * C.view(-1, 1)
        W = self.act(self.lin_cat(torch.cat([W, q_r], dim=-1)))
        v = self.lin(v)
        e = v[j] * W
        return e
...
class Q_SchNet(torch.nn.Module):
    r"""
    ...
    def forward(self, batch_data):
        ...
        for update_e, update_v in zip(self.update_es, self.update_vs):
            e = update_e(v, dist, dist_emb, edge_index, q_r)
